...
<ul>
	<c:choose>
		...
		<c:when test="${step eq '2'}">
			<li>
				<h3>c:import Tag : jsp 'include'와 유사. 서버 내부 또는 서버 외부의 모든 자원 컨텐츠를 포함하는 추가 기능 있음</h3>
		...
		<c:when test="${step eq '7'}">
			<li>
				<h3>c:param Tag : 포함하는 impor, url 태그의 URL에게 매개 변수를 추가</h3>
		...
		<c:when test="${step eq '11'}">
			<li>
				...
				<c:set var="String" value="Welcome to JSP programing"/>
				<c:if test="${fn:startsWith(String,'Welcome')}">
				<p>String starts with Welcome</p>
				</c:if>
				
				<c:if test="${fn:startsWith(String,'programing')}">
				<p>String starts with programing</p>
				</c:if>
			</li>
		</c:when>
		<c:when test="${step eq '12'}">
			<li>
				...
				<c:set var="String" value="Welcome to JSP programing"/>
				<c:if test="${fn:endsWith(String,'programing')}">
				<p>String ends with programing</p>
				</c:if>
        ...
        <c:when test="${step eq '15'}">
        	<li>
        		...
        		<c:set var="string" value="Welcome JSP Programming"/>
        ...
        <c:when test="${step eq '17'}">
        	<li>
        		<h3>fn:substring() : 주어진 시작 및 끝 위치에따라 문자열의 하위 집합을 반환</h3>
